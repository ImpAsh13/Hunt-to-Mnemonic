...
#### Изменения от 30.03.22!
    Добавлена поддержка поиска ELECTRUM STANDART (-elec)
#### Изменения от 12.02.22!
    Добавлена поддержка поиска ELECTRUM OLD SEED (-elec)
...
#### Изменения от 29.01.22!
    Добавлен режим Random (-rnd). генерируется случайный ключ и от него последовательно проверяются 500 приватников. (на скорость почти не влияет)
...
#### создание BIP39 Mnemonic для 10 языков. Возможно использовать все сразу или какие-то отдельно 
    ...
    4 варианта поиска (BIP32, BIP44, ETH, BTC)
        ...
        - Режим ETH создан для поиска кошельков ethereum и ethereum classic
        - Режим combo совмещает в себе режимы BTC и ETH
...
#### Установка:
    ...
    pip install bip32
    pip install bitcoinlib # нужен для создания H160 если вы используете готовые блюм фильтры от меня, то он не нужен!
...
#### Ключи использования  (Проверьте свои БАТНИКИ, аргументы для запуска изменились)
    ...
    -elec включение генерации и проверки старых мнемоник кошелька electrum (общая скорость сильно падает!!!)
